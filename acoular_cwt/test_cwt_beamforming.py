...
# Adjust path to find acoular_cwt in the current directory or package
current_dir = Path(__file__).parent
if str(current_dir) not in sys.path:
    sys.path.append(str(current_dir))

try:
    from acoular_cwt.spectra import WaveletSpectra
except ImportError:
    try:
        from spectra import WaveletSpectra
    except ImportError:
        print("Could not import WaveletSpectra from acoular_cwt.spectra or local spectra.py")
        sys.exit(1)


def create_test_signal_if_missing(filename, fs=51200, duration=16.0):
    ...
    # 3. 创建移动声源
    # 定义移动轨迹，在 8.0s 时刻实现位置跳变
    # 增加定义点以实现更清晰的跳变
    # 0-7.95s: 位于 (-0.15, -0.15, -0.3)
    # 8.0-16.0s: 位于 (0.15, 0.15, -0.3)
    t1 = ac.Trajectory()
    t1.points = {
        0.0: (-0.15, -0.15, -0.3),
        7.95: (-0.15, -0.15, -0.3),  # 保持在左下方
        8.0: (0.15, 0.15, -0.3),  # 跳变到右上方
        16.0: (0.15, 0.15, -0.3)  # 保持在右上方
    }
...
def run_beamforming_test():
    ...
    # 2. 实例化 WaveletSpectra
    print("Instantiating WaveletSpectra...")
    # 目标频率 2000Hz (CWT将只计算这个频率的CSM)
    mws = WaveletSpectra(source=ts, target_freq=2000.0, smoothing_width=0)
    ...
    # 4. 测试两个不同的时间点并绘图
    # 声源在 8.0s 发生位置跳变
    # t=4.0s 时应在 (-0.15, -0.15)
    # t=12.0s 时应在 (0.15, 0.15)
    test_times = [4.0, 12.0]

    plt.figure(figsize=(10, 5))

    for i, t in enumerate(test_times):
        expected_pos = "(-0.15, -0.15)" if t < 8.0 else "(0.15, 0.15)"
        print(f"\n{'=' * 60}")
        print(f"Computing Beamforming map at t={t}s")
        print(f"  Expected source position: {expected_pos}")
        print(f"{'=' * 60}")

        # 更新 WaveletSpectra 的时间
        mws.target_time = t

        # [CRITICAL FIX]
        # Acoular 的 BeamformerBase 会缓存结果。它依赖于 freq_data.digest。
        # 如果 WaveletSpectra 的 digest 机制没有完美地将 target_time 的变化
        ...
        # 调用 bb.synthetic()
        # 参数1: 频率 (Must match target_freq because WaveletSpectra only returns that freq)
        # 参数2: 带宽 (设为 0 或 3 这里的实现其实 WaveletSpectra 只有一个频率，
        # Acoular 的 synthetic 会在其返回的 fftfreq 中查找这个频率)
        
        # 注意: bb.synthetic(f, oct) 会去 freq_data.fftfreq() 里找在 f 周围的频率。
        # 我们的 WaveletSpectra 只返回 [2000.0]。
        # 所以查询 2000 应该能命中。
        try:
